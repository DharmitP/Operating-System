...
//            	paddr = as->pagetable[(int)(faultaddress / PAGE_SIZE)].page_frame_num * PAGE_SIZE;
            ...
            //page not found in pagetable, allocate page in coremap and update pagetable and load corresponding segment from disk
            if(found == 0){
                paddr = getppages(1);
                if(paddr == 0){
                    splx(spl);
                ...
                //determine filesize to read
                if((((faultaddress - vbase_text) / PAGE_SIZE) + 1) == as->as_npages_text){//check if fault address is in the last page of text
                    filesize = as->as_sizeof_text - ((as->as_npages_text - 1) * PAGE_SIZE);
                }
                else
                    filesize = PAGE_SIZE;

                /* Open the file. */
                result = vfs_open(as->progname, 0, &v);
                if (result) {
                    return result;
                }
                
                //load segment from disk and write to memory
                result = load_segment(v, faultaddress - vbase_text, faultaddress, 
				      PAGE_SIZE, filesize, 1);
		if (result) {
			return result;
            }
...
//                paddr = as->pagetable[(int)(faultaddress / PAGE_SIZE)].page_frame_num * PAGE_SIZE;
            ...
            if(found == 0){
                ...
                //determine filesize to read
                if((((faultaddress - vbase_data) / PAGE_SIZE) + 1) == as->as_npages_data){//check if fault address is in the last page of text
                    filesize = as->as_sizeof_data - ((as->as_npages_data - 1) * PAGE_SIZE);
                }
                else
                    filesize = PAGE_SIZE;
