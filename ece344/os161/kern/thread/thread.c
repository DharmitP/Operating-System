...
//ASST2 variables and data structures
...
thread_create(const char *name)
{
	...
	// them here.
        ...
        thread->pid = pid_count;
        
        //create a new process
        struct process *newProc;
        
        //check if process is first one spawned, if so, parent_pid = 0
        if(pid_count == 1)
            newProc = create_process(pid_count, 0, thread, 0, NULL, NULL, 0);
        else
            newProc = create_process(pid_count, curthread->pid, thread, 0, NULL, NULL, 0);
        append_process(newProc);
...
/* ASST2 Code starts here*/

//Create a new process and set values, called from threadcreate
struct process* create_process(pid_t pid, pid_t parent_pid, struct thread *p_thread, int has_exited, int exit_code, struct process *next_process, int was_waited_on){
    //create new process
    struct process* newProc = (struct process*)kmalloc(sizeof(struct process));
    //see if process was able to be created
    if(newProc == NULL){
        kprintf("Error: Proc create");
        return NULL;
    }
    
    //assign values to new process
    newProc->pid = pid;
    newProc->parent_pid = parent_pid;
    newProc->process_thread = p_thread;
    newProc->has_exited = has_exited;
    newProc->exit_code = exit_code;
    newProc->next = next_process;
    newProc->was_waited_on = was_waited_on;
...
void append_process(struct process* newProc){
    //corner case for if list is empty
    if(process_head == NULL){
        process_head = newProc;
        return;
    }
    
    //traverse to last process added
...
struct process* find_process(pid_t search_pid){
    
    //traverse linked list until you find process with given pid
    struct process *temp = process_head;
    while(temp != NULL){
        if(temp->pid == search_pid){
            return temp;
        }
        else
            temp = temp->next;
...
void asst2_bootup(){
    //process ids start from 1 and increment by 1 each time
    pid_count = 1;
    
    calling_waitpid_from_kern = 0; //initially assume waitpid is always called from usermode
    
    waitpid_lock = lock_create("waitpid_lock");
    waitpid_cv = cv_create("waitpid_cv");
    
    procLL_sem = sem_create("procLL_sem", 1);
    write_sem = sem_create("write_sem", 1);
    read_sem = sem_create("read_sem", 1);
}
