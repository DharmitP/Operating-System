...
common_prog(int nargs, char **args)
{
	...
	if (result) {
        ...
        int retval;
        int status = 1;
        calling_waitpid_from_kern = 1; //call waitpid from kernel mode
        err_code = sys_waitpid(prog_thread->pid, &status, 0, &retval);
...
{
	int bit = atoi(args[1]);  // convert 2nd arg to int (returns 0 if NaN)
	int valid = 1;
	// Checking if arguments are valid
	if(nargs != 3)  // # of args must be exactly 3
		valid = 0;
	if(bit <= 0 || bit > 12) 
		valid = 0;
